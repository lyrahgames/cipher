...
SCENARIO("AES-128 In-Place and Function-Style Interface Correctness") {
  ...
  constexpr size_t n = 100'000;
  for (size_t i = 0; i < n; ++i) {
    const auto key = aes128::random_key(rng);
    const auto round_keys = aes128::expansion(key);
    const auto block = aes128::random_block(rng);
    aes128::block_type check;
    auto b = block;

    // Generate reference encryption.
    cipher.encrypt(round_keys, block, check);

    // In-Place Encryption Interface
    cipher.encrypt(round_keys, b);
    REQUIRE(check == b);
    // In-Place Decryption Interface
    cipher.decrypt(round_keys, b);
    REQUIRE(block == b);

    // Function-Style Encryption Interface
    const auto c = cipher.encryption(round_keys, block);
    REQUIRE(check == c);
    // Function-Style Decryption Interface
    const auto d = cipher.decryption(round_keys, c);
    REQUIRE(block == d);
  }
}
